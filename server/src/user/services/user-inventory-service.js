...
class UserInventoryService {
  ...
  async getInventory(userId) {
    ...
    const localizedItems = await localizeNestedItems(userId, inventory.items);
  ...
  async useItem(userId, itemId) {
    ...
    const itemEntryIndex = inventory.items.findIndex((entry) => entry.item.toString() === itemId);
    ...
    const localizedItemName = await this.localizeItemName(userId, item.name);

    return {
      success: true,
      messages: [await inventoryNotifications.useItem(userId, localizedItemName)],
      currentHp: userStats.hp,
