...
class TasksService {
  ...
  createTask({ text, deadline, difficulty }, userId) {
    validateUserId(userId);
  ...
  async completeTask(taskId, userId) {
    ...
    const isEligibleToDamage = boss && task.deadline && new Date(task.deadline) > now;
    ...
    const { stats, message: levelUpMessage } = await userStatsService.gainExperience(userId, xp);
    ...
    if (bossResult?.messages) {
      messages.push(...normalizeMessages(bossResult.messages));
    }
  ...
  async applyOverduePenalty(taskId, userId) {
    ...
    const { stats, message: hpZeroMessage } = await userStatsService.takeDamage(userId, damage);
    ...
    const boss = await bossService.getBoss(userId);
    const createdAfterBossSpawned = boss && task.deadline && task.createdAt > boss.spawnedAt;

    if (createdAfterBossSpawned) {
      const rageResult = await bossService.addRage(userId, [taskId]);

      if (rageResult?.messages?.length) {
        messages.push(...normalizeMessages(rageResult.messages));
      }
    }
