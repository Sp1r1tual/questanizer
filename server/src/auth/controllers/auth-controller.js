...
const activate = async (req, res, _next) => {
  try {
    await authService.activate(req.params.link);

    res.type("html").send(activationSuccessHTML(`${process.env.CLIENT_URL}/login`));
  } catch (error) {
    res
      .status(400)
      .type("html")
      .send(activationErrorHTML(`${process.env.CLIENT_URL}/login`, error.message));
  }
};
...
export { registration, login, logout, refresh, activate, forgotPassword, resetPassword };
