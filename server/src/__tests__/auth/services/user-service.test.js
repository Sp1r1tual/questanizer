...
describe("UserService", () => {
    ...
    const mockUserDto = new UserDto(mockUserDoc);
    ...
    describe("login", () => {
        it("should login with valid credentials", async () => {
            ...
            const mockUserDto = new UserDto(mockUser);
    ...
    describe("updateUserProfile", () => {
        ...
        it("should throw if user not found", async () => {
            ...
            await expect(
                userService.updateUserProfile(userId, { username: "newUser" })
            ).rejects.toThrow("User not found");
        });

        it("should throw if new username is already taken", async () => {
            UserModel.findById.mockResolvedValue(existingUser);
            UserModel.findOne.mockResolvedValue({ _id: "other-id" });
        ...
        it("should keep old values if no new data provided", async () => {
            UserModel.findById.mockResolvedValue(existingUser);
            UserModel.findOne.mockResolvedValue(null);
            ...
            expect(existingUser.bio).toBe("old bio");
            expect(existingUser.save).toHaveBeenCalled();
            expect(result).toEqual(expect.objectContaining({ id: userId }));
        });
    });
